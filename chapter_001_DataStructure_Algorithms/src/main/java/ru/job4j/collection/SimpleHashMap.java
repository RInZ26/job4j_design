...
public class SimpleHashMap<K, V> implements Iterable<SimpleHashMap.Node<K, V>> {
    ...
    public boolean insert(K key, V value) {
        ...
        if (table[indexOfTable] != null) {
            ...
            if (checkRuleInInnerLinkedList(deepNode.iterator(),
                                           o -> o.hash == newNode.hash
                                                   && o.key.equals(newNode.key))
                    != null) {
                return false;
            } else {
    ...
    public V get(K key) {
        ...
        if (table[index] != null) {
            ...
            Node<K, V> nodeInLinkedList = checkRuleInInnerLinkedList(
                    nodeIterator, o -> o.hash == hash && o.key.equals(key));
    ...
    public boolean delete(K key) {
        ...
        if (table[index] != null) {
            Iterator<Node<K, V>> nodeIterator = table[index].iterator();
            Node<K, V> nodeRoadToDelete = checkRuleInInnerLinkedList(
                    nodeIterator, o -> o.hash == hash && o.key.equals(key));
    ...
    public Iterator<Node<K, V>> iterator() {
        ...
        return new Iterator<Node<K, V>>() {
             ...
             */
            private int currentVersionOfCollection = SimpleHashMap.this.versionOfCollection;
            /**
             * Количество элементов, пройденных итератором
             */
            private int countOfGotElements = 0;
            /**
             * Указатель на текущий индекс в таблице
             */
            private int pointerOnIndexOfTable = 0;
            /**
             * Если ячейка в таблице является связным списком, то указывает на
             * следующую ноду
             */
            private Iterator<Node<K, V>> iteratorOfTheCurrentInnerLinkedList = Collections
                    .emptyIterator();

            /**
             * hasNext, который кроме всего остального осуществляет сдвиги поинтеров.
             * Сначала идёт проверка по таблице, как только обнаружена неNull строка
             * смотрим -  итератор был проинициализирован ранее ?
             * Если нет - мы встретили строку впервые, следовательно инициализируем
             * итератор, иначе идём по нему дальше (уже в next)
             */
            @Override
            public boolean hasNext() {
                ...
                while ((table[pointerOnIndexOfTable] == null
                    ...
                    iteratorOfTheCurrentInnerLinkedList =
                            table[pointerOnIndexOfTable] == null
                                    ? Collections.emptyIterator()
                                    : table[pointerOnIndexOfTable].iterator();
                }
    ...
    public static class Node<K, V> implements Iterable<Node<K, V>> {
        ...
        public Iterator<Node<K, V>> iterator() {
            ...
            return new Iterator<Node<K, V>>() {
                 ...
                 */
                private Node<K, V> pointerCurrentElement = new Node<K, V>(
                        Node.this);
                /**
                 * Указатель на элемент перед pointerCurrentElement, то есть
                 * на элемент перед элементом, который нужно удалить
                 */
                private Node<K, V> pointerBeforeCurrentElement = new Node<>(
                        pointerCurrentElement);
                /**
                 * Флаг, удаляли ли мы после next или нет. Не допускает повторные
                 * вызовы remove без вызова next
                 */
                ...
                /**
                 * Т.к. remove можно вызывать только после Next, проверяем его
                 * на Null, а после удаления зануляем.
                 * Сперва перенаправляем основную связь у поинтера некст
                 * Потом уже "ломаем" старую связь через pointerBeforeNext.next.next
                 *
                 * В первом else рассматривается ситуация подробно описанная в
                 * SimpleHashMap.delete() - случай, когда
                 * удаляется сама голова списка и больше в списке ничего нет
                 * Если голова не единственный элемент, тогда происходит переприсвоение
                 * key и value (if(hasNext()))
                 *
                 * Если вызвано до next или дважды после одного некс - выбросит
                    ...
                    if (!removed) {
                        if (pointerCurrentElement.equals(Node.this)) {
                            if (hasNext()) {
                                ...
                                pointerCurrentElement = new Node<K, V>(
                                        Node.this);
                                pointerBeforeCurrentElement = new Node<K, V>(
                                        pointerCurrentElement);
                            } else {
