...
public class ArgZip {
	...
	if (argZip.initIfValid()) {
	    ...
	    new Zip().packFiles(Search.seekByPredicate(argZip.directory,
						       getExcludeThisExtensionPredicate(
							       argZip.exclude)),
				argZip.output);
	}
    }
    ...
    /**
     * Всё ли ок с args Без реглуярок ОСОЗНАННО => примитивные сплиты. Второй if
     * на IllegalArgumentException - одновременно проверяет и ключи и наличие
     * параметра Выглядит убого конечно. В этом же методе идёт инициализация
     * полей класса, потому что строки уже заспличены - нет смысла делать это
     * дважды.
     */
	...
	if (!splitedDirectory[0].equals("-d") || !splitedExclude[0]
		.equals("-e") || !splitedOutput[0].equals("-o")
		|| splitedDirectory.length != 2 || splitedExclude.length != 2
		|| splitedOutput.length != 2) {
	    throw new IllegalArgumentException(
		    "One of parameters is empty or using wrong key. U have to init  -d=\"sourseDirectory\" -e=\"excludeExtension\" -o=\"outputZipFile\"");
	}
	if (isExistAndDirectory(splitedDirectory[1]) && isRightOutput(
		splitedOutput[1])) {
